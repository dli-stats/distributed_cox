...
def data_from_csv(csv_file: str):
    ...
    x_headers = ["A"] + list(sorted((k for k in dataframe.keys() if k.startswith("X") and len(k) > 1), key=lambda s: int(s[1:])))
    X = dataframe[x_headers]
    delta = dataframe["status"]
    T = dataframe["time"]
    group_labels = dataframe["indDP"] - 1
    X, delta, T, group_labels = map(lambda x: x.to_numpy(), (X, delta, T, group_labels))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--data-dir', type=str, default='../distributed_multivariablecox_data/')
    ...
    if convert_data:
        ...
        eq1_solver = jit(eq1.get_eq1_solver(use_ad=True, solver_max_steps=40, norm_stop_thres=1e-1))
        beta_guess = onp.random.normal(0, 0.1, (X.shape[1], ))
        print("Guess:", beta_guess)
        beta_correct = onp.array([-0.35044839,  0.02395054, -0.01438011, 0.10463269, -0.19862842, 
            -0.23871021, -0.04685478,  0.10718658,  0.32139027,  
            0.14003561, 0.34813819, -0.24538269, -0.07905443,  0.02583216,  0.09991671])

        X_normalized, initial_beta_normalized, scale = varderiv.data.normalize(X, beta_guess)
