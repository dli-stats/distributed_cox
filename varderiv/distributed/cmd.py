...
Usage:
    ...
    cmd.py local-send-T <DATA_FILE> [options]
    cmd.py master-send-T <DATA_DIR> [options]
    cmd.py local-send-vals <T_DELTA_FILE> <DATA_FILE> [options]
    cmd.py master-analytics <DATA_DIR> [options]

Options:
  -h --help               Show this screen.
  --version               Show version.
  --key=<key>             Use integer random key.  [default: 0]
  --eq=<eqn>              Which equation type to use.
...
def run_master_analytics(args):
  ...
  beta_hat, beta_corrected_cov = distributed_compute_eq2_master(*local_vals)
  print("beta:", beta_hat)
