...
def run_dummy_gen(args):
  ...
  K = int(args['<K>'])
  k1, _ = jrandom.split(args['--key'])
  group_sizes = group_sizes_generator(N, K, "same")
  T, X, delta, _, group_labels = data_generator(N,
                                                X_DIM,
                                                group_sizes,
                                                return_T=True)(k1)
  if args['save_global']:
...
def run_local_send_vals(args):
  ...
  npzfile = onp.load(args['<DATA_FILE>'])
  T_group, X_group, delta_group = (npzfile['T_group'], npzfile['X_group'],
                                   npzfile['delta_group'])
  # TODO randomized guess
  if args['eq2']:
    T_delta = onp.load(args['<T_DELTA_FILE>'])['T_delta']
    local_vals = distributed_compute_eq2_local(T_group,
                                               X_group,
                                               delta_group,
                                               T_delta,
                                               solve_eq1_fn=solve_eq1_fn)
  elif args['eq4']:
    local_vals = distributed_compute_eq4_local(T_group,
                                               X_group,
                                               delta_group,
                                               solve_eq1_fn=solve_eq1_fn)
  else:
...
def run_master_analytics(args):
  solver_max_steps, solver_thres = int(args['--solver-max-steps']), float(
      args['--solver-thres'])
  ...
  if eq in ('eq1', 'eq3'):
    ...
    if eq == 'eq1':
      ...
      X_groups, delta_groups = group_data_by_labels(1, K, X, delta,
                                                    group_labels)
    ...
    local_val_files = glob.glob(os.path.join(args['<DATA_DIR>'],
                                             "local_vals_*"))
  ...
  pd.DataFrame(beta_hat).to_csv(
      os.path.join(save_dir, "result_{eq}_beta_hat.csv".format(eq=eq)))
  pd.DataFrame(beta_cov).to_csv(
      os.path.join(save_dir, "result_{eq}_beta_cov.csv".format(eq=eq)))
