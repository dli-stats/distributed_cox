...
def solve_newton(fn,
  ...
  InternalState = collections.namedtuple(
      "InternalState", "guess value step last_valid_u halving")
  ...
  def newton_update(state: InternalState):
    ...
    is_finite = np.logical_and(np.all(np.isfinite(u)),
                               np.all(np.isfinite(new_value)))

    return jax.lax.cond(is_finite, (u, new_guess, new_value, state.step),
                        do_normal_update, state, do_halving)
  ...
  initial_state = InternalState(initial_guess, initial_value, 0,
                                np.zeros_like(initial_guess), 1)
